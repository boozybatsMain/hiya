...
Очередь задач на следующую пересборку кампаний — [`docs/ads-rebuild-todo.md`](docs/ads-rebuild-todo.md).
Перед пересборкой агент обязан показать пользователю этот список и спросить, какие
задачи выполнять. Новые идеи правок кампании добавлять туда, а не применять сразу
(правки адсета перезапускают обучение — меняем пакетом).
